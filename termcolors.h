...
#define ANSI_WHITE   "\x1B[0;37m"	// actually rendered as light grey, legible
									// even on a white background. See
									// ANSI_HIGHLIGHT_WHITE for real white.
...
#define _ANSI_HIGHLIGHT_YELLOW  "\x1B[0;1;33m"	// se fix below
...
/* Currently _ANSI_HIGHLIGHT_YELLOW is not displayed well on some terminals.
   So we define ANSI_HIGHLIGHT_YELLOW with a better choice. */
#define ANSI_HIGHLIGHT_YELLOW   ANSI_HIGHLIGHT_KHAKI3
...
typedef enum ColorMode {
	...
	COLOR_ON,		/**< All colors, no restrictions. */
	COLOR_16,		/**< Only the base 16 ANSI colors. */
	COLOR_256,		/**< Only 256 colors. */
	COLOR_24BIT,	/**< For truecolor or 24bit color support. */
	_COLOR_INVALID = -EINVAL,	/*!< Unable to determine a valid mode. */
} ColorMode;

bool on_tty(void);
bool terminal_is_dumb(void);
ColorMode parse_colors_env(const char *envvar);
ColorMode get_color_mode(void);
bool underline_enabled(void);
static bool colors_enabled(void);

/**
 * Check if colors are considered supported on stdout.
 * @return false if colors should not be used, true otherwise.
 */
...
#define DEFINE_ANSI_FUNC_UNDERLINE_256(name, NAME, FALLBACK)		\
	static inline const char *ansi_##name(void) {					\
		switch (get_color_mode()) {									\
			case COLOR_OFF: return "";								\
			case COLOR_16: return underline_enabled()				\
				? ANSI_##FALLBACK ANSI_UNDERLINE : ANSI_##FALLBACK;	\
			default : return underline_enabled()					\
				? ANSI_##NAME ANSI_UNDERLINE: ANSI_##NAME;			\
		}															\
	}
